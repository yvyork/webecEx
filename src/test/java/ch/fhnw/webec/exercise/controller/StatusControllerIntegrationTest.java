...
public class StatusControllerIntegrationTest {
    ...
    public void testShowStatus() throws Exception {
        ...
        // when
        when(this.statusRepository.findById(statusId)).thenReturn(Optional.of(status));

        // then
        this.mockMvc.perform(get("/statuses/{id}/", statusId))
                .andExpect(status().isOk())
                .andExpect(content().string(containsString("1")))
                .andExpect(content().string(containsString("new")));

        verify(this.statusRepository, times(1)).findById(statusId);
    }
    ...
    public void testAddOrEditStatus() throws Exception {
        ...
        // then
        this.mockMvc.perform(get("/statuses/{id}/edit?", statusId))
                .andExpect(status().isOk())
                .andExpect(content().string(containsString("1")))
                .andExpect(content().string(containsString("new")));
    ...
    public void testDeleteStatus() throws Exception {
        ...
        // then
        this.mockMvc.perform(post("/statuses/{id}/delete", statusId))
